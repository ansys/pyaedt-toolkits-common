...
Usually it’s found in: .github /workflows/ci ci-cd.yaml.

Why . yaml and GitHub Actions are important?
...
.yaml, ``runs-on: ubuntu-latest/windows-latest`` means “use a runner with
Ubuntu/Windows installed” or ``runs-on: self-hosted`` means “use a runner that I setup
myself”.

In a .yaml file, ``runs-on`` tells the system what kind of machine (runner) should execute
the job.

``ubuntu-latest`` is a virtual machine (VM) provided by GitHub where everything is pre-configured
and ready to go.
